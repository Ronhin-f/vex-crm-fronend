...
import { useAuth } from "../context/AuthContext"; // si migrás a hook JS, cambia a ../hooks/useAuthFromLocalStorage
...
export default function ClientesDashboard() {
  ...
  const agregarCliente = async () => {
    ...
    try {
      // Enviamos las claves que tu backend espera hoy.
  ...
  const editarCliente = async (cliente) => {
    ...
    const nuevoContacto = window.prompt("Nuevo contacto:", cliente.contacto || "");
    const nuevaCategoria = window.prompt("Nueva categoría:", cliente.categoria || "");
  ...
  const clientesFiltrados = clientes.filter((c) => {
    ...
    const con = (c?.contacto || "").toLowerCase();
    const cat = (c?.categoria || "").toLowerCase();
    return !txt || n.includes(txt) || con.includes(txt) || cat.includes(txt);
  });
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      <input
        ...
        className="input input-bordered input-sm mb-6 w-full"
      />
      ...
      {clientesFiltrados.length === 0 ? (
        ...
        <div className="grid gap-4">
          {clientesFiltrados.map((cliente) => (
            <div
              key={cliente.id}
              className="bg-orange-50 p-4 rounded-xl shadow flex items-center justify-between"
            >
              <div>
                ...
                <p className="text-sm text-gray-600 flex gap-2 mt-1">
                  {cliente.contacto && (
                    <span className="flex items-center gap-1">
                      <Phone size={14} /> {cliente.contacto}
                    </span>
                  )}
                  {cliente.categoria && (
                    <span className="flex items-center gap-1">
                      <Tag size={14} /> {cliente.categoria}
                    </span>
                  )}
                </p>
              </div>
